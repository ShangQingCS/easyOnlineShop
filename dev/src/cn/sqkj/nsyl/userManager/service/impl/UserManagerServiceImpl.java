...
public class UserManagerServiceImpl implements IUserManagerService{
	...
	public PageBean queryNsUserTeamList(PageBean pageBean,String id) throws Exception {
		...
		StringBuffer sql = new StringBuffer(" select distinct *  from " +
				"( select n2.* from ns_user  n1,ns_user  n2 where n1.id= "+id+" and n2.user_pid=n1.id " +
	...
	public PageBean queryNsUserCount(PageBean pageBean)
		...
		StringBuffer sql = new StringBuffer(" select DISTINCT t1.true_name,t1.user_name,t1.user_sex,t1.user_grade as level,"
				+ " t1.user_phone,t1.identity_card,t1.create_time,t1.user_status,t1.identity_status,t1.user_grade as grade,t1.user_fx_balance, "
	...
	public PageBean queryUserTiXianList(PageBean pageBean) throws Exception {
		StringBuffer sql = new StringBuffer(" select t.id,t.true_name,t.user_name,t1.trade_amount,t1.user_id,t1.trade_type,t1.pay_open_bank,t1.pay_account,  ");
		sql.append(" (select t2.name from ns_dictionaries t2 where t2.`code`=t1.trade_type and t2.type='TRADE_TYPE') type_name  ");
		sql.append(" from ns_user t,ns_user_purse t1 where 1=1 and t.id = t1.user_id and t.tixian_status = 1 ");
		List params = new ArrayList();
		//查询总计路数
		int total = userManagerDAO.findNsUserCount(sql.toString(), params);
		//查询数据集
