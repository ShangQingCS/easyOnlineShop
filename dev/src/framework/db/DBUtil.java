...
public class DBUtil {
	...
	public static void main(String[] args) throws  Exception {
		System.out.println(DBUtil.class.getName());
	}
	 ...
	 * @return
	 */
	public Integer getSequece(String seqName){
		Query query = this.session.createSQLQuery("select "+seqName.toUpperCase()+".nextval from dual");
	 ...
	 * @return
	 */
	public List queryByHql(String hql, Map params, int pageIndex , int pageSize){
	 ...
	 * @return
	 */
	public int queryByHqlCount(String hql, Map params){
		Query query = this.session.createQuery(hql);
        if(params!=null && params.size()>0){
            Iterator iter = params.keySet().iterator();
            while(iter.hasNext()){
	...
	/**
	 * 插入操作时默认值赋值（数据库列:lr_sj,yx_bj,user_type,lrry_dm）
	 * @param pojo
	 */
	public static void setDefaultValueByInsert(Object pojo){
		setPojoValue(pojo, "LrSj", java.sql.Timestamp.class, DateUtils.getTimestamp());
		setPojoValue(pojo, "YxBj", String.class, SysDict.FLAG_ACT);
		...
		if(user!=null){
			setPojoValue(pojo, "LrryDm", String.class, user.getUId());
			setPojoValue(pojo, "UserId", String.class, user.getUId());
			setPojoValue(pojo, "CreateUserId", String.class, user.getUId());
			setPojoValue(pojo, "CreateOrgId", String.class, user.getDeptId());
			
		}
	}
	
	/**
	 * 修改操作时默认值赋值（数据库列:xg_sj）
	 * @param pojo
	 */
	public static void setDefaultValueByUpdate(Object pojo){
		setPojoValue(pojo, "XgSj", java.sql.Timestamp.class, DateUtils.getTimestamp());
	...
	public static DBUtil getDBUtilByRequest(){
		...
		if(request.getAttribute(DB_SESSION_NAME) == null || !(request.getAttribute(DB_SESSION_NAME) instanceof DBUtil)){
