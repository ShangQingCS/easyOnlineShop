...
public class ExportExcelAction extends PageAction {
	...
	public HSSFWorkbook exportExcel(PageBean resultData) throws Exception {
		...
		if (resultData != null && resultData.getTotal() > 0) {
			...
			this.createCell(row, 8, style, HSSFCell.CELL_TYPE_STRING, "分红余额");
			this.createCell(row, 9, style, HSSFCell.CELL_TYPE_STRING, "分红金额");
			this.createCell(row, 10, style, HSSFCell.CELL_TYPE_STRING, "消费金额");
			this.createCell(row, 11, style, HSSFCell.CELL_TYPE_STRING, "积分");
			this.createCell(row, 12, style, HSSFCell.CELL_TYPE_STRING, "当前余额");
			this.createCell(row, 13, style, HSSFCell.CELL_TYPE_STRING, "是否激活会员");
			this.createCell(row, 14, style, HSSFCell.CELL_TYPE_STRING, "会员等级");
			this.createCell(row, 15, style, HSSFCell.CELL_TYPE_STRING, "状态");
			...
			for(Iterator iterator = resultData.getPageData().iterator();iterator.hasNext();){
				...
				if (map.get("userFxBalance")!=null) {//user_fx_balance
					this.createCell(row1, 8, style, HSSFCell.CELL_TYPE_STRING, map.get("userFxBalance"));
				}
				if (map.get("fhje")!=null) {
					this.createCell(row1, 9, style, HSSFCell.CELL_TYPE_STRING, map.get("fhje"));
				}
				if (map.get("xf")!=null) {
					this.createCell(row1, 10, style, HSSFCell.CELL_TYPE_STRING, map.get("xf"));
				}
				if (map.get("jf")!=null) {
					this.createCell(row1, 11, style, HSSFCell.CELL_TYPE_STRING, map.get("jf"));
				}
				if (map.get("dqye")!=null) {
					this.createCell(row1, 12, style, HSSFCell.CELL_TYPE_STRING, map.get("dqye"));
				}
				if (map.get("grade")!=null) {
					if(map.get("grade").equals('0')){
						this.createCell(row1, 13, style, HSSFCell.CELL_TYPE_STRING, "否");
					}else{
						this.createCell(row1, 13, style, HSSFCell.CELL_TYPE_STRING, "是");
					}
				}
				if (map.get("level")!=null) {
					this.createCell(row1, 14, style, HSSFCell.CELL_TYPE_STRING, "level"+map.get("level"));
				}
				if (map.get("userStatus")!=null) {
	            	if(map.get("userStatus").equals('0')){
	            		this.createCell(row1, 15, style, HSSFCell.CELL_TYPE_STRING, "注销");
	            	}else if(map.get("userStatus").equals('1')){
	            		this.createCell(row1, 15, style, HSSFCell.CELL_TYPE_STRING, "正常");
	            	}else if(map.get("userStatus").equals('2')){
	            		this.createCell(row1, 15, style, HSSFCell.CELL_TYPE_STRING, "冻结");
	            	}
	            }
			}
